// Скрипт для генерации двухфидерных нереверсивных ящиков управления Я5000
const fs = require('fs');
...
// Типы фидеров (для двухфидерных нет варианта с контактами)
const feederTypes = [
...
function calculatePrice(brand, current, feederType) {
  ...
  if (current <= 1) basePrice = 15000;
  else if (current <= 2.5) basePrice = 16500;
  else if (current <= 6) basePrice = 18500;
  else if (current <= 12) basePrice = 21000;
  else if (current <= 25) basePrice = 24500;
  else if (current <= 40) basePrice = 29000;
  else if (current <= 63) basePrice = 35000;
  else basePrice = 43000;
  ...
  for (const current of nominalCurrents) {
    for (const feeder of feederTypes) {
      ...
      const product = {
        ...
        reversible: false,
        base_price: price,
        description: `Ящик управления Я5121 двухфидерный нереверсивный ${feeder.name} ${current}А ${brand}`,
        full_description: `Ящик управления электродвигателем Я5121 двухфидерный нереверсивный ${feeder.name}, номинальный ток ${current}А, производитель ${brand}`,
