  ...
  Alpine.data('productData', () => {
    return {
      ...
      getCommutationDescription() {
        if (this.commutationType === 'control_cabinet') {
          // Для шкафов управления - возвращаем описание шкафов управления
          return {
            purpose: `Шкаф управления предназначен для автоматического управления электродвигателями различной мощности. Основные функции:`,
            purposeList: [
              'плавный пуск и остановка электродвигателя для снижения пусковых токов;',
              'защита двигателя от перегрузок, коротких замыканий и обрыва фаз;',
              'контроль параметров работы двигателя (ток, напряжение, температура);',
              'возможность дистанционного и местного управления;',
              'индикация режимов работы и аварийных состояний.'
            ],
            workflow: [
              {
                title: 'Режим плавного пуска',
                text: 'При подаче команды "Пуск" устройство плавного пуска постепенно увеличивает напряжение на двигателе; пусковой ток ограничивается до 2-4 номинальных значений; после выхода на номинальные обороты двигатель переключается на прямое питание от сети.'
              },
              {
                title: 'Рабочий режим',
                text: 'Двигатель работает от сети напрямую через байпасные контакторы; система контроля отслеживает параметры работы; при превышении уставок срабатывает защита.'
              },
              {
                title: 'Режим остановки',
                text: 'При подаче команды "Стоп" или срабатывании защиты двигатель отключается от сети; возможна плавная остановка с контролируемым снижением оборотов.'
              }
            ],
            features: [
              {
                name: 'Плавный пуск',
                desc: '— Снижение пусковых токов в 2-3 раза, защита оборудования'
              },
              {
                name: 'Многоуровневая защита',
                desc: '— Защита от перегрузки, КЗ, обрыва фаз, перегрева'
              },
              {
                name: 'Удобство управления',
                desc: '— Местное и дистанционное управление, индикация состояния'
              },
              {
                name: 'Энергоэффективность',
                desc: '— Оптимизация энергопотребления, снижение износа оборудования'
              },
              {
                name: 'Надёжность',
                desc: '— Промышленное исполнение, высокий ресурс работы'
              },
              {
                name: 'Универсальность',
                desc: '— Подходит для различных типов нагрузок и применений'
              }
            ]
          };
        } else if (false) {
