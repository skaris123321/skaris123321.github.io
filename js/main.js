    ...
    return {
      ...
      getCommutationDescription() {
        if (this.commutationType === 'contactors') {
          return {
            purpose: `Шкаф АВР (автоматического ввода резерва) на базе контакторов предназначен для бесперебойного электроснабжения потребителей. Он обеспечивает:`,
            purposeList: [
            ...
            workflow: [
              ...
              },
              {
                ...
                text: 'Контроллер АВР фиксирует отклонение параметров (например, падение напряжения ниже уставки); через заданную выдержку времени отключает контактор основного ввода; включает контактор резервного ввода; загорается индикатор <strong>«Резервный ввод»</strong>.'
              },
              {
                title: 'Возврат в нормальный режим (восстановление основного ввода)',
                text: 'После стабилизации напряжения на основном вводе контроллер АВР с выдержкой времени переключает нагрузку обратно; отключает контактор резервного ввода; восстанавливает индикацию <strong>«Основной ввод»</strong>.'
              }
            ],
            features: [
              {
                name: 'Контакторная схема',
                desc: '— Высокая надежность и долговечность коммутации'
              },
              {
                name: 'Программируемый контроллер',
                desc: '— Гибкие настройки логики работы и временных задержек'
              },
              {
                name: 'Раздельные контакторы',
                desc: '— Независимое управление каждым вводом'
              },
              {
                name: 'Механическая блокировка',
                desc: '— Исключает одновременное включение контакторов'
              },
              {
                name: 'Расширенная диагностика',
                desc: '— Контроль состояния контакторов и цепей управления'
              },
              {
                name: 'Универсальность',
                desc: '— Подходит для больших токов и сложных схем'
              }
            ]
          };
        } else {
